...
function handleLogin(e) {
  ...
  const users = JSON.parse(localStorage.getItem('myFinanceUsers') || '[]');
  const user = users.find(u => u.email === email && u.password === password);
  
  if (user) {
    // Set current user (don't store password in state)
...
function handleRegister(e) {
  ...
  users.push(newUser);
  localStorage.setItem('myFinanceUsers', JSON.stringify(users));
